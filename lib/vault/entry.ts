...
export type EntryType = 'login' | 'note' | 'card' | 'identity' | 'passkey';

/** Passkey-specific fields (stored when type === 'passkey') */
export interface PasskeyFields {
  credentialId: string;       // base64url-encoded WebAuthn credential ID
  publicKey: string;          // base64url-encoded COSE public key
  publicKeyAlgorithm: number; // COSE algorithm ID (-7 = ES256, -8 = EdDSA, -257 = RS256)
  rpId: string;               // relying party ID (e.g. "github.com")
  rpName: string;             // relying party display name (e.g. "GitHub")
  userHandle?: string;        // base64url-encoded user handle from RP
  transports?: string[];      // e.g. ["internal", "hybrid", "usb"]
  aaguid?: string;            // authenticator AAGUID (hex, 32 chars)
  signCount?: number;         // last known signature counter
  attestationObject?: string; // base64url-encoded raw attestation (for VC issuance)
  clientDataJSON?: string;    // base64url-encoded clientDataJSON (for VC issuance)
}
...
export interface VaultEntry {
  ...
  passkey?: PasskeyFields; // present when type === 'passkey'
}

/** Fields that get individually encrypted in the vault bundle */
export type EncryptedFields = 'username' | 'password' | 'notes' | 'passkeyJson';
...
export interface VaultEntryEncrypted {
  ...
  enc: {
    ...
    password?: string;
    notes?: string;
    passkeyJson?: string; // JSON-serialized PasskeyFields (encrypted)
  };
}
...
export function getEncryptableFields(entry: VaultEntry): Record<string, string> {
  ...
  if (entry.passkey) {
    fields.passkeyJson = JSON.stringify(entry.passkey);
  }
  return fields;
}
...
export function mergeDecryptedFields(
  ...
  if (decrypted['passkeyJson']) {
    try { entry.passkey = JSON.parse(decrypted['passkeyJson']); } catch { /* ignore corrupt passkey data */ }
  }
  return entry;
}
