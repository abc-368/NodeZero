/**
 * redeemer.ts — Anonymous token redemption for upload authorization
 *
 * Calls POST /v2/tokens/redeem with a token+signature pair (NO DID headers).
 * Returns a one-time "nz-token-*" authorization string valid for 60 seconds.
 *
 * This is the core privacy mechanism: the Worker verifies the RSA signature
 * but cannot link the token back to the DID that obtained it.
 */
