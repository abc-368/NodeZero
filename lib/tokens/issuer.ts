/**
 * issuer.ts — Batch blind token issuance
 *
 * Calls POST /v2/tokens/issue with a DID-signed request containing
 * blinded tokens. Unblinds the server's responses, verifies each
 * signature, and stores the final tokens in the local pool.
 *
...
/** Cached parsed mint public key. */
let _mintPubKey: MintPublicKey | null = null;
function getMintPubKey(): MintPublicKey {
  if (!_mintPubKey) _mintPubKey = parseMintPublicKey(MINT_PUBLIC_KEY_JWK);
  return _mintPubKey;
}
...
/**
 * Request a batch of blind-signed tokens from the backend.
 *
 * Flow:
 *   1. Compute how many tokens to request (budget − current pool)
 *   2. Generate + blind that many tokens
 *   3. DID-sign the issuance request
 *   4. POST /v2/tokens/issue
 *   5. Unblind + verify each response
...
async function _refillPoolImpl(did: string): Promise<RefillResult | RefillError> {
  ...
  // Step 1: Generate blinded tokens
  const prepared = Array.from({ length: count }, () =>
    prepareBlindedToken(mintPubKey)
  );

  // Step 2: Build request body
  const blindedTokensB64 = prepared.map(p => bytesToBase64url(p.blindedBytes));
  ...
  const response = await fetch(`${SYNC_API_BASE}/v2/tokens/issue`, {
    ...
    body: JSON.stringify({ blindedTokens: blindedTokensB64, count }),
  });
  ...
  // Step 5: Unblind + verify each signed token
  const now = Math.floor(Date.now() / 1000);
  const tokens: StoredNodeZeroToken[] = [];

  for (let i = 0; i < result.issued; i++) {
    try {
      const S_blind_bytes = base64urlToBytes(result.signedBlindedTokens[i]);
      const { token, signature: sig } = finalizeToken(
        S_blind_bytes,
        prepared[i].blindingFactor,
        prepared[i].tokenBytes,
        mintPubKey,
      );
      tokens.push({
        version: 1,
      ...
      console.warn(`[NodeZero] Token ${i} unblind/verify failed:`, err);
