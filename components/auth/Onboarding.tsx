...
import { escapePopupForWebAuthn, restoreSidePanelPreference } from '@/lib/popout';
...
export function Onboarding({ onComplete, onRecover }: OnboardingProps) {
  ...
  const handleRegisterPasskey = useCallback(async () => {
    ...
    try {
      ...
      if (msg.includes('cancelled') || msg.includes('NotAllowed') || msg.includes('not allowed') || msg.includes('timed out')) {
        setError('Registration was dismissed. Tap the button to try again.');
      } else {
  ...
  const handleVerify = useCallback(async () => {
    ...
    try {
      ...
      // Capture mnemonic phrase for HD wallet (will be encrypted in vault bundle)
      const mnemonicForWallet = mnemonicPhrase;
      ...
      // Seal vault with both tiers (mnemonic is AES-GCM encrypted in the bundle)
      ...
      const session = { bundle, entries: [], primaryKey, recoveryKey, mnemonic: mnemonicForWallet };
      ...
      // Restore side panel preference (may have been temporarily overridden)
