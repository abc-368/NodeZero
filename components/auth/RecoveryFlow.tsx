...
type RecoveryState = 'input' | 'looking-up' | 'confirm-overwrite' | 'deriving' | 'pin-setup' | 'done' | 'error' | 'archived';
...
export function RecoveryFlow({ onRecovered, onBack }: RecoveryFlowProps) {
  ...
  // Pending overwrite: stored when mnemonic DID doesn't match local vault
  const pendingOverwriteRef = useRef<{
    cleaned: string;
    did: string;
    derivedKeyPair: { keyPair: CryptoKeyPair; did: string; bipSeed: Uint8Array };
  } | null>(null);
  ...
  /** User confirmed overwrite — fetch remote vault for the entered mnemonic's DID */
  const handleConfirmOverwrite = useCallback(async () => {
    const pending = pendingOverwriteRef.current;
    if (!pending) return;

    const { cleaned, did, derivedKeyPair } = pending;
    ...
    try {
      ...
      const remoteBundle = await lookupAndDownloadVault(did);

      if (!remoteBundle) {
        setError(
          'No remote vault found for this recovery phrase. ' +
          'Make sure you have synced that vault at least once.'
        );
        setState('error');
        return;
      }

      if (remoteBundle.did !== did) {
        setError('Remote vault DID mismatch. Recovery cannot proceed.');
        setState('error');
        return;
      }

      console.log('[NodeZero] Remote vault found for different identity. Starting PBKDF2\u2026');
      setProgress(15);

      // PBKDF2 in Web Worker
      setState('deriving');
      ...
      const vaultSession = await unsealWithRecoveryKey(remoteBundle, recoveryKey, cleaned);
      setProgress(95);

      // Store DID key pair
      await initializeDID(recoveryKey, derivedKeyPair, derivedKeyPair.bipSeed);

      // Overwrite local vault with the remote one
      await saveVaultToStorage(remoteBundle);
      setProgress(100);

      // PIN/biometric setup if needed
      if (remoteBundle.kdfParams.primary.type === 'pin' || remoteBundle.kdfParams.primary.type === 'biometric') {
      ...
      console.error('[NodeZero] Overwrite recovery error:', err);
      setError(err?.message ?? 'Recovery failed. Check your phrase and try again.');
      setState('error');
      setProgress(0);
    }
  }, [onRecovered, cleanupWorker, finalizeRecovery]);
  ...
  const handleRecover = useCallback(async () => {
    ...
    try {
      ...
      if (bundle.did !== did) {
        // Local vault belongs to a different identity — ask user to confirm overwrite
        pendingOverwriteRef.current = { cleaned, did, derivedKeyPair };
        setState('confirm-overwrite');
        setProgress(0);
        return;
      }
      ...
      setStatusMsg('Decrypting vault\u2026');
      const vaultSession = await unsealWithRecoveryKey(bundle, recoveryKey, cleaned);
      setProgress(95);
  ...
  return (
    <Layout>
      <Header
        title="Recover Vault"
        left={
          state === 'input' || state === 'error' || state === 'confirm-overwrite' ? (
            ...
            }} className="h-6 w-6 p-0" aria-label="Back">
      ...
      {state === 'confirm-overwrite' ? (
        /* ── Confirm overwrite of local vault ────────────────────────────── */
        <>
          <ScrollableBody className="p-4 space-y-4">
            ...
            <div className="space-y-1">
              <h2 className="text-sm font-semibold">Different Vault Detected</h2>
              <p className="text-xs text-muted-foreground">
                This recovery phrase belongs to a different vault than the one
                currently stored on this device.
              </p>
            </div>
            <div className="flex items-start gap-2 bg-amber-500/10 text-amber-700 dark:text-amber-400 border border-amber-500/20 rounded-md px-3 py-2">
              <AlertCircle className="w-4 h-4 shrink-0 mt-0.5" />
              <p className="text-xs">
                Proceeding will <strong>replace</strong> the local vault with the one
                from this recovery phrase. The current local vault data will be lost
                unless it has been synced to the cloud.
              </p>
            </div>
            <p className="text-xs text-muted-foreground">
              Your new vault will be downloaded from remote storage using this phrase.
            </p>
          </ScrollableBody>
          <Footer>
            <div className="space-y-2">
              <Button
                ...
                className="w-full"
              >
                Replace Local Vault & Recover
              </Button>
              <Button
                onClick={() => { pendingOverwriteRef.current = null; setState('input'); }}
                variant="outline"
                className="w-full"
              >
                Cancel
