...
> **Beta Software** — NodeZero is under active development and should be considered beta.
...
### Anonymous Sync (VOPRF Tokens)

Most password managers track exactly when and how often you sync. NodeZero uses a **Verifiable Oblivious PRF (VOPRF, RFC 9497)** to decouple sync metering from your identity:
...
2. **Blinding** (client-side): Token inputs are hashed to a P-256 curve point and blinded with a random scalar — the server evaluates the PRF without seeing the actual token values.
3. **Redemption** (anonymous): When syncing, the extension redeems a token with **no identifying headers**. The server can verify the token is legitimate but cannot link it back to the issuance request. Unlinkability is formally proven via DLEQ proof.
...
- **Link email**: right-click on any Gmail page → NodeZero → Link this email to my identity — registers a SHA-256 hash of your address so others can encrypt to you (free, 0 tokens). **Important:** this step is not automatic. Each user must explicitly link their email address before others can send them encrypted messages. Without linking, senders have no way to discover the recipient's public key, and the message will be sent in plaintext. This is by design — email-to-DID mappings are never created without the user's consent, preserving anonymity for those who choose not to participate
...
### Privacy
- **Anonymous sync metering** — unlinkable token redemption via VOPRF (P-256, Privacy Pass RFC 9497, `@noble/curves`)
...
### Wallet
- **Multi-chain HD wallet** — derived from your recovery phrase (BIP-44 for EVM, BIP-84/86 for Bitcoin)
- **5 EVM chains** — Ethereum, Base, Arbitrum, Optimism, Polygon with native + ERC-20 token balances
- **Bitcoin** — SegWit (BIP-84) and Taproot (BIP-86) addresses with balance and transaction history
- **EIP-1193/6963 provider** — NodeZero acts as a browser wallet for dApps (MetaMask-compatible)
- **In-extension token swap** — Uniswap v4 on Base with Permit2 approval flow
- **Transaction signing** — EIP-1559 transaction and typed-data signing with approval UI
- **ENS resolution** — forward and reverse ENS lookups

### Collaboration
- **Delegation VCs** — securely share vault access with other NodeZero users via X25519 key wrapping and Ed25519-signed Verifiable Credentials
- **Security report** — password health analysis (weak, reused, old) with a composite 0–100 security score powered by `zxcvbn`

### Usability
- **Dashboard landing page** — security score, vault stats, and non-zero token balances at a glance after unlock
- **Side panel mode** — open NodeZero as a persistent Chrome side panel (Chrome 114+) with full-height layout; toggle between popup and side panel in settings
...
- **Pop-out window** — expand the extension into a resizable standalone window for larger screens
- **Cross-identity vault recovery** — recover a different vault on the same device with a confirmation warning before overwriting
...
| Anonymous sync tokens | VOPRF P-256 (Privacy Pass, RFC 9497, `@noble/curves`) |
| HD wallet | [@scure/bip32](https://github.com/paulmillr/scure-bip32) + [@scure/btc-signer](https://github.com/nicephil/btc-signer) (audited) |
